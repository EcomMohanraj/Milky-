...
function DashboardContent() {
  ...
  const filteredOrders = orders.filter((o) =>
    o.id.toLowerCase().includes(orderSearchQuery.toLowerCase().trim())
  );
          ...
          {isRegistering ? (
            ...
            <form onSubmit={loginForm.handleSubmit(handleLogin)} className="flex flex-col gap-4">
              ...
              <div className="p-3 bg-muted/40 rounded-lg text-[10px] text-muted-foreground leading-normal mt-2">
                ...
                - Admin: <code className="font-semibold text-primary">admin@milkymushrooms.com</code> (pw: Admin@123)<br />
    ...
    <div className="container mx-auto px-4 md:px-6 py-10 flex-grow flex flex-col md:flex-row gap-8 pb-20">
              ...
              <div className="flex flex-col gap-5">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border/60 pb-3">
                  ...
                  <div className="relative w-full sm:max-w-xs">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 border border-border rounded-xl text-xs bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-mono"
                    />
                  </div>
                </div>
                ...
                {orders.length === 0 ? (
                  ...
                  <div className="py-12 text-center flex flex-col items-center gap-4">
                    <ShoppingBag className="w-12 h-12 text-muted-foreground" />
                    <p className="text-xs text-muted-foreground italic leading-normal">
                      No orders matching &ldquo;{orderSearchQuery}&rdquo; were found.
                    </p>
                    <button
                      onClick={() => setOrderSearchQuery("")}
                      className="px-6 py-2.5 bg-secondary text-secondary-foreground font-bold text-xs rounded-xl hover:bg-secondary/80"
                    >
                      Clear Search
                    </button>
                  </div>
                  ...
                  <div className="flex flex-col gap-4">
                    {filteredOrders.map((ord) => (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap justify-between items-center gap-2 border-b border-border/60 pb-3">
                          <div className="flex flex-col gap-0.5">
                            <button
                              ...
                              className="font-mono text-primary font-bold hover:underline cursor-pointer text-left focus:outline-none"
                              title="Click to view full details and live status"
                            >
                              Order ID: {ord.id}
                            </button>
                        ...
                        {/* Full details (Address and Payment) */}
                        <div className="border-t border-border/60 pt-3 flex flex-col gap-1 text-[10px] text-muted-foreground bg-muted/5 p-2 rounded-xl">
                          <p><span className="font-bold text-foreground">Delivery Address:</span> {ord.address}</p>
                          {ord.payment_id && (
                            <p><span className="font-bold text-foreground">Payment Transaction ID:</span> <span className="font-mono text-foreground font-semibold">{ord.payment_id}</span></p>
                          )}
                        </div>
      ...
      {selectedOrderDetails && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-all duration-300">
          <div className="bg-card border border-border/80 w-full max-w-2xl rounded-3xl shadow-2xl flex flex-col overflow-hidden max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className="p-6 border-b border-border/60 flex justify-between items-center bg-muted/20">
              <div>
                <span className="text-[10px] font-black uppercase tracking-wider text-primary">Live Tracking & Details</span>
                <h3 className="text-lg font-extrabold text-foreground font-outfit mt-1">
                  Order Details
                </h3>
              </div>
              <button
                onClick={() => setSelectedOrderDetails(null)}
                className="p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground rounded-xl transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex flex-col gap-6 text-xs text-foreground">
              {/* Order ID & Date banner */}
              <div className="flex flex-wrap justify-between items-start gap-4 bg-muted/40 border border-border/60 p-4 rounded-2xl">
                <div className="flex flex-col gap-1.5">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Order Reference</span>
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-bold text-foreground text-sm selection:bg-primary/20">{selectedOrderDetails.id}</span>
                    <button
                      onClick={() => handleCopyId(selectedOrderDetails.id)}
                ...
                <div className="flex flex-col gap-1 sm:text-right">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Purchase Date</span>
                  <span className="font-bold flex items-center gap-1 mt-0.5 justify-end">
              ...
              <div className="flex flex-col gap-4">
                <span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Logistics Milestones</span>
                
                {/* Horizontal / Vertical stepper */}
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3 pt-2">
                  {["pending", "confirmed", "processing", "packed", "shipped", "delivered"].map((stage, idx) => {
                    const currentStatus = selectedOrderDetails.status.toLowerCase();
                    const stagesOrder = ["pending", "confirmed", "processing", "packed", "shipped", "delivered"];
                    const activeIndex = stagesOrder.indexOf(currentStatus);
                    ...
                    const IconComponent = stageIcons[stage] || Clock;

                    return (
                      <div
                        key={stage}
                        className={`p-3 border rounded-2xl flex flex-col items-center text-center gap-2 transition-all ${
                          isCompleted
                            ? "bg-primary/5 border-primary/40 text-foreground"
                            : "bg-muted/10 border-border text-muted-foreground opacity-60"
                        } ${isActive ? "ring-2 ring-primary/40 scale-[1.02] shadow-sm font-bold bg-primary/10 border-primary" : ""}`}
                      >
                        ...
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[8px] text-muted-foreground leading-tight">
                            {isCompleted ? (isActive ? "Current" : "Done") : "Pending"}
                          </span>
                        </div>
                      </div>
                    );
              ...
              {/* Delivery & Payment Section (2 cols) */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Shipping Details */}
                <div className="border border-border/80 p-4 rounded-2xl bg-muted/5 flex flex-col gap-2.5">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-primary" /> Delivery Destination
                  </span>
                  <div className="text-xs leading-relaxed">
                    <p className="font-semibold text-foreground">Recipient Location:</p>
                    <p className="text-muted-foreground mt-1 bg-background p-2.5 border border-border/40 rounded-xl">
                      {selectedOrderDetails.address || "No shipping address associated."}
                    </p>
                  </div>
                </div>
                ...
                <div className="border border-border/80 p-4 rounded-2xl bg-muted/5 flex flex-col gap-2.5">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-mono font-semibold text-foreground truncate max-w-[150px]" title={selectedOrderDetails.payment_id}>
                        {selectedOrderDetails.payment_id || "Mock Sandbox Transaction"}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-xs border-t border-border/40 pt-2">
                      ...
                      <span className="px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-600 border border-emerald-200">
                        {selectedOrderDetails.status}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-xs border-t border-border/40 pt-2">
                      <span className="text-muted-foreground">Amount Charged:</span>
                      <span className="font-extrabold text-sm text-foreground">₹{selectedOrderDetails.amount.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground flex items-center gap-1">
                  <ShoppingBag className="w-3.5 h-3.5 text-primary" /> Purchased Items
                </span>
                <div className="border border-border/60 rounded-2xl overflow-hidden">
                  <div className="divide-y divide-border/60 bg-muted/5">
                    {selectedOrderDetails.items?.map((item) => (
                      <div key={item.id} className="p-4 flex items-center justify-between gap-4 text-xs hover:bg-muted/10 transition-colors">
                        <div className="flex items-center gap-3">
                          {item.product?.image && (
                            <img
                              src={item.product.image}
                              alt={item.product.name}
                              className="w-10 h-10 object-cover rounded-lg border border-border/80 shadow-sm shrink-0"
                            />
                          )}
                          <div className="flex flex-col gap-0.5">
                            <span className="font-bold text-foreground">{item.product?.name || "Organic Milky Mushrooms"}</span>
                        ...
                        <div className="text-right flex flex-col gap-0.5 shrink-0">
                          ...
                          <span className="font-extrabold text-primary">₹{(item.price * item.quantity).toFixed(2)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {/* Modal Footer */}
            <div className="p-5 border-t border-border/60 bg-muted/20 flex justify-end gap-3">
              <button
                onClick={() => setSelectedOrderDetails(null)}
                className="px-5 py-2.5 bg-secondary text-secondary-foreground font-bold rounded-xl hover:bg-secondary/80 transition-colors text-xs"
              >
                Close Details
              </button>
              <Link
                href={`/track-order?id=${selectedOrderDetails.id}`}
                className="px-5 py-2.5 bg-primary text-primary-foreground font-black uppercase tracking-wider rounded-xl hover:bg-primary/95 transition-colors text-xs flex items-center gap-1.5"
              >
                Live Tracking Page
              </Link>
            </div>
