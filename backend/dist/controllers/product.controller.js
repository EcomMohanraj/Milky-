...
exports.productController = {
    ...
    async create(req, res) {
        try {
            ...
            if (!resolvedCategoryId && category) {
                let catRecord = await prisma_1.prisma.category.findUnique({ where: { name: category } });
                if (!catRecord) {
                    // Auto create category if not exists
                    catRecord = await prisma_1.prisma.category.create({
                        data: {
                            name: category,
    ...
    async update(req, res) {
        try {
            const { id } = req.params;
            const { name, slug, description, image, price, stock, category, categoryId, nutrition } = req.body;
            const current = await prisma_1.prisma.product.findUnique({ where: { id } });
            if (!current) {
                return res.status(404).json({ success: false, error: "Product not found." });
            }
            const data = {};
